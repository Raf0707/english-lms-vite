  ...
  return (
    <div className="test-runner">
      <header className="test-runner__header">
        <div><span>Проверка знаний</span><h2>{test.title}</h2></div>
      ...
      <div className="test-runner__questions">
        {test.questions.map((question, index) => {
          ...
          return (
            <Card key={question.id} className={`test-question ${submitted && !manual ? (correct ? 'test-question--correct' : 'test-question--wrong') : ''}`}>
                ...
                {question.type === 'single' && question.options ? (
                  <div className="test-options">
                    {question.options.map((option) => (
                      <label key={option}>
                        <input type="radio" name={question.id} value={option} checked={current === option} onChange={(event) => setAnswers((state) => ({ ...state, [question.id]: event.target.value }))} disabled={submitted}/>
                        <span>{option}</span>
                      </label>
                    ))}
                  </div>
                  ...
                  <div className="test-options">
                    {question.options.map((option) => {
                      ...
                      return <label key={option}><input type="checkbox" value={option} checked={selected} disabled={submitted} onChange={(event) => setAnswers((state) => { const prev = Array.isArray(state[question.id]) ? state[question.id] as string[] : []; return { ...state, [question.id]: event.target.checked ? [...prev, option] : prev.filter((item) => item !== option) }; })}/><span>{option}</span></label>;
                    })}
                  </div>
                ) : question.type === 'essay' ? (
                  <textarea className="test-text-answer test-text-answer--essay" rows={6} value={typeof current === 'string' ? current : ''} onChange={(event) => setAnswers((state) => ({ ...state, [question.id]: event.target.value }))} placeholder="Введите развёрнутый ответ" disabled={submitted}/>
                ) : (
                  <input className="test-text-answer" value={typeof current === 'string' ? current : ''} onChange={(event) => setAnswers((state) => ({ ...state, [question.id]: event.target.value }))} placeholder={question.type === 'matching' ? 'Введите соответствия' : 'Введите ответ'} disabled={submitted}/>
                )}
                {submitted ? (
                  <div className={`test-feedback ${manual ? 'test-feedback--manual' : ''}`}>
                    {manual ? <Clock3 size={18}/> : correct ? <CheckCircle2 size={18}/> : <CircleAlert size={18}/>} 
                    <span>{manual ? 'Ответ отправлен преподавателю на ручную проверку.' : correct ? 'Верно' : `Правильный ответ: ${expected.join(', ')}`}{!manual && question.explanation ? ` · ${question.explanation}` : ''}</span>
                  </div>
                ) : null}
              </div>
            </Card>
      ...
      <footer className="test-runner__footer">
        {submitted ? (
          result.manual > 0 ? <div className="test-result test-result--pending"><div><strong>{result.totalAuto ? `${result.score}%` : '✓'}</strong><span>автоматическая часть</span></div><p>{result.manual} ответ(а) отправлено на ручную проверку. Итоговый статус появится после проверки преподавателем.</p></div> :
          <div className={`test-result ${result.passed ? 'test-result--passed' : 'test-result--failed'}`}><div><strong>{result.score}%</strong><span>{result.correct} из {result.totalAuto} ответов</span></div><p>{result.passed ? 'Отлично! Тест пройден, можно переходить дальше.' : 'Попробуйте ещё раз. Перед новой попыткой можно вернуться к материалам урока.'}</p></div>
        ) : <span>Ответьте на все вопросы и отправьте результат.</span>}
        {submitted ? <Button variant="secondary" icon={<RotateCcw size={17} />} onClick={reset}>Пройти ещё раз</Button> : <Button onClick={submit} disabled={answeredCount < test.questions.length}>Отправить ответы</Button>}
      </footer>
    </div>
  );
}
